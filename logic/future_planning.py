...
def recommend_strategy(summary_df, mode, target):
    ...
    if mode == 'Ad Spend Budget':
        ...
        if current_overall_roi > 0:
            ...
            current_revenue = df['revenue'].sum() # Assuming df['revenue'] exists from original df passed to MMM
            if current_revenue > 0:
                revenue_ratio = target / current_revenue
                df['Planned Spend'] = df['Spend'] * revenue_ratio
            else:
                st.error("Cannot calculate plan for revenue target: Current revenue is zero.")
                df['Planned Spend'] = df['Spend'] # No change if cannot calculate
